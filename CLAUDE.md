...
- **游戏定位**：一款治愈系 2D 手绘风移动端竖屏养成游戏。玩家在浮空小岛上照护一颗蛋，孵化出第一只宠物 Shell Pudding，并通过场景化互动持续培养亲密度。
...
- **语言**：GDScript（不使用 C#）。
- **目标平台**：移动端**竖屏**。PC 仅用于开发调试，鼠标/键盘是触摸的临时替代输入，实现交互时必须同时考虑触摸路径。
- **画布基准**：1080×1920（9:16），`stretch/mode="canvas_items"` + `aspect="expand"`，`handheld/orientation=5`（sensor portrait）。
  因 `expand` 的作用，画布尺寸随屏幕比例变化——20:9 超长屏是 1080×2400，4:3 平板是 1440×1920，横屏窗口是 3413×1920。
  **禁止写死画布宽高**，一律读 `get_viewport().get_visible_rect().size`；布局靠锚点，不靠绝对坐标。
- **横屏已作废**：设计文档早期按横屏 1920×1080 描述，2026-09 起全面改为竖屏（9:16），冲突处以本规范为准。
...
项目已启用 15 个插件。使用时**优先调用插件能力，不要重复造轮子**：
...
| script-ide、godot_ai | 编辑器增强工具，**不参与运行时逻辑**，运行时代码不得依赖它们 |
...
1. **Phase 1 — 主岛原型**：移动端竖屏浮空岛，蛋巢、食盆、暖灯、清洁物、玩具、小游戏入口齐备，占位美术但交互可用。（当前进度）
...
- 主岛在移动端竖屏布局下保持干净、可交互；20:9 / 19.5:9 / 16:9 / 4:3 四种比例下锚点均正确落在安全区内。
- 小游戏入口在主岛可见或可明确发现。
